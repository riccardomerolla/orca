...
class OrcaOverridesTest extends munit.FunSuite:
  ...
  test("flow uses a custom FsTool when supplied"):
    ...
    var observed: String = ""
    flowWith(fs = Some(fake), interaction = silentInteraction) {
  ...
  test("flow uses a custom ClaudeTool when supplied"):
    ...
    var observed: String = ""
    flowWith(claude = Some(fakeClaude), interaction = silentInteraction) {
  ...
  test("flow collects extra listeners alongside the interaction's"):
    ...
    flowWith(interaction = interaction, extraListeners = List(tracker)) {
      summon[FlowContext]
